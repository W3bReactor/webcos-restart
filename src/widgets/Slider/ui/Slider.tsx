...
export const Slider = ({data}: ISlider) => {
    ...
    return (
        <section className={styles.sliderWrapper}>
            <Swiper slideClass={styles.slide} onUnlock={() => setLocked(false)} onLock={() => setLocked(true)}
                    modules={[Autoplay]} ref={sliderRef} slidesPerView={1} speed={1000} loop={true} spaceBetween='60px'
                    autoplay={{
                        delay: 5000
                    }}>
                {data.map(item =>
                            ...
                            <div className={styles.sliderContent}>
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        color: !hover ? item.color : "#fff",
                                        backgroundColor: !hover ? "transparent" : item.color
            ...
            <div className={`${styles.sliderButtons} ${locked ? styles.noSwiping : ""}`}>
                <button className={`${styles.sliderArrowBtn} ${styles.sliderPrev}`}
                        onClick={() => (sliderRef.current as unknown as SwiperRef).swiper.slidePrev()}>
                    <Image src={SliderArrowIcon} alt={'Предыдущий'}/>
                </button>
                <button className={`${styles.sliderArrowBtn} ${styles.sliderNext}`}
                        onClick={() => (sliderRef.current as unknown as SwiperRef).swiper.slideNext()}>
